...
public class SaveAllDataJobRoundTrip implements Job {
    ...
    public void execute(JobExecutionContext context) throws JobExecutionException {
        // Retrieve the file path and date range from the job data
        String filePath = context.getJobDetail().getJobDataMap().getString("filePath");
        String startDateStr = context.getJobDetail().getJobDataMap().getString("startDate");
        String endDateStr = context.getJobDetail().getJobDataMap().getString("endDate");
        ...
        try (InputStream is = Files.newInputStream(Paths.get(filePath))) {
            List<roundTrip> trips = parseExcelFile(is, startDate, endDate);
            repository.saveAll(trips);
            System.out.println("Saved all data to roundTrip table for date range " + startDate + " to " + endDate);
        } catch (IOException e) {
    ...
    private List<roundTrip> parseExcelFile(InputStream is, LocalDate startDate, LocalDate endDate) throws IOException {
        ...
        while (rows.hasNext()) {
            ...
            while (cellsInRow.hasNext()) {
                ...
                switch (cellIdx) {
                    case 0:
                        // ID column in Excel - ignored, DB id is auto-generated
                        break;
                    case 1:
                    ...
                    case 9:
                        ...
                        break;
                    case 14:
                        // 'Eartiga' column in Excel -> ertiga field in entity
                        trip.setErtiga((int) getCellValueAsNumeric(currentCell));
                        break;
                    default:
                        break;
                }
